...
namespace Negocio
{
    public class SocioNegocio
    {
        ...
        //2
        public double promEdadEquipo(List<Socio> socios, string equipo)
        {
            double prom;
            int cont = socios.Where(s => s.Equipo == equipo).Count();
            int sum = socios.Where(s => s.Equipo == equipo).Sum(so => so.Edad);
        ...
        //3
        public List<Socio> listarEstadoCivilEstudios(List<Socio> socios, string estadoCivil, string nivelEstudios)
        {
            List<Socio> sociosCasadosUniversitarios = new List<Socio>();

            sociosCasadosUniversitarios = socios.Where(s => s.EstadoCivil == estadoCivil && s.NivelDeEstudios == nivelEstudios).ToList();
        ...
        //4
        public List<string> listarNombres(List<Socio> socios, string equipo)
        {
            List<string> nombresRiver = new List<string>();
            List<string> nombresRiverSR = new List<string>();

            nombresRiver = socios.Where(s => s.Equipo == equipo).Select(n => n.Nombre).ToList();
